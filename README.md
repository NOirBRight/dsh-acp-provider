...
The Node-only activity-store export owns versioned append-only history and filesystem safety; each adapter supplies its existing event decoder. Sequence allocation is cached per session after one validation read, so appending is O(batch) and independent per session. `read()` stays the validating bootstrap and binding read; `readAfter(sessionId, afterSeq, limit)` adds bounded incremental pages (ordered records, next cursor, continuation) under fixed record and byte limits. Browser code must not import this subpath. Binding lookup is `latestNativeSessionBinding` on the browser-safe contracts export and returns branded `ExternalAgentSessionRef`. History remains readable without a native runtime.
